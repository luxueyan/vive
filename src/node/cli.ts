...
function logHelp() {
  console.log(`
Usage: vive [command] [args] [--options]

Commands:
  vive                       Start server in current directory.
  vive serve [root=cwd]      Start server in target directory.
  vive build [root=cwd]      Build target directory.
...
async function resolveOptions(mode: string) {
  ...
  // normalize root
  // assumes all commands are in the form of `vive [command] [root]`
